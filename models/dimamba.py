...
class Block(nn.Module):
    ...
    def forward(
        ...
        if self.modulate and time_embeds is not None:
            (shift_msa, scale_msa, gate_msa) = self.adaLN_modulation(time_embeds)[:, None].chunk(3, dim=-1)
        ...
        if self.modulate and time_embeds is not None:
            ...
            hidden_states = bias_dropout_scale_fn(hidden_states, None, gate_msa, residual, self.dropout)
...
class BiMambaEmbeddings(nn.Module):
    def __init__(
        ...
        self.word_embeddings = nn.Embedding(input_dim, config.d_model, **factory_kwargs)
...
class BiMambaMixerModel(nn.Module):
    def __init__(
        ...
        self.embeddings = BiMambaEmbeddings(config, input_dim=input_dim, **factory_kwargs)
        ...
        if self.temb_strategy and "adaln" in self.temb_strategy:
            self.adaLN_modulation_final = nn.Linear(config.d_temb, 2 * d_model, bias=True)
            self.adaLN_modulation_final.weight.data.zero_()
            self.adaLN_modulation_final.bias.data.zero_()

        norm_f = (nn.LayerNorm if not config.rms_norm else RMSNorm)(d_model, eps=config.norm_epsilon, **factory_kwargs)
        self.norm_f = norm_f
    ...
    def pre_apply_temb(self, input_embeds, time_embeds):
        ...
        if self.temb_strategy == "concat":
            input_embeds = torch.cat(
                [time_embeds.unsqueeze(1).tile(1, input_embeds.shape[1], 1), input_embeds], axis=-1
            )
    ...
    def forward(
        ...
        if time_embeds is not None and self.temb_strategy in ["concat", "add"]:
        ...
        for ind, layer in enumerate(self.layers):
            ...
            layer_out = layer(hidden_states, residual, inference_params=None, time_embeds=time_embeds)
        ...
        if not self.fused_add_norm:
            ...
            if time_embeds is not None and self.temb_strategy and "adaln" in self.temb_strategy:
                shift, scale = self.adaLN_modulation_final(time_embeds)[:, None].chunk(2, dim=2)
...
class BiMambaForMaskedLM(BiMambaPreTrainedModel):
    ...
    def __init__(self, config: BiMambaConfig, device=None, dtype=None, **kwargs):
        ...
        if not config.tie_word_embeddings and config.temb_strategy == "concat":
