...
class ProbaEmbedder(nn.Module):
    """
    Embeds a scalar probability into a vector representation.
    Logit transformation transforms the input probability into an unbounded space.
    """

    def __init__(self, hidden_size, method="logit"):
        super().__init__()

        assert method in ["logit", "erfinv", "raw", "bucket"], f"Unknown method: {method}"
...
class ConditionEmbedder(nn.Module):
    def __init__(self, method, cond_dim, **kwargs):
        super().__init__()
        self.method = method
        ...
        if method == "label":
            ...
            self.model = LabelEmbedder(num_classes, cond_dim, dropout_prob)
        elif method == "proba":
            proba_method = kwargs.pop("proba_method")
            self.model = ProbaEmbedder(cond_dim, proba_method)
...
class DIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(self, config, vocab_size: int):
        ...
        if hasattr(config.model, "conditional") and config.model.conditional and hasattr(config.model, "cond_method"):
            if config.model.cond_method == "label":
                assert hasattr(config.model, "num_classes") and hasattr(config.model, "label_dropout")
                assert config.model.num_classes == config.data.num_classes, "num_classes must match the dataset"
                kwargs = {"num_classes": config.model.num_classes, "dropout_prob": config.model.label_dropout}
                print("Using label condition method")
            elif config.model.cond_method == "proba":
                assert hasattr(config.model, "proba_method")
                kwargs = {"proba_method": config.model.proba_method}
    ...
    def forward_with_cfg(self, indices, sigma, labels, cfg_scale):
        if self.cond_embed.method == "label":
            ...
            out = self.forward(indices, sigma, labels)
            cond, uncond = torch.split(out, out.size(0) // 2, dim=0)
            return uncond + cfg_scale * (cond - uncond)

        else:
            warnings.warn("Conditional method is not label. Ignoring cfg_scale.")
            return self.forward(indices, sigma, labels)
