...
class Diffusion(L.LightningModule):
    def __init__(self, config, tokenizer: transformers.PreTrainedTokenizer):  # noqa: PLR0915
        super().__init__()
        ...
        if self.config.training.ema > 0:
            self.ema = models.ema.ExponentialMovingAverage(
                itertools.chain(self.backbone.parameters(), self.noise.parameters()), decay=self.config.training.ema
            )
        else:
            self.ema = None
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        checkpoint["loops"]["fit_loop"]["epoch_loop.state_dict"]["_batches_that_stepped"] = checkpoint["loops"][
            "fit_loop"
        ]["epoch_loop.automatic_optimization.optim_progress"]["optimizer"]["step"]["total"]["completed"]
    ...
    def forward(self, x, sigma, labels=None):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            if self.backbone.training or cfg is None or labels is None:
                ...
                logits = self.backbone.forward_with_cfg(
                    x, sigma, labels=labels, cfg_scale=self.config.sampling.cfg_scale
                )
    ...
    def _compute_loss(self, batch, prefix):
        ...
        losses = self._loss(batch["input_ids"], attention_mask, labels)
        loss = losses.loss
    ...
    def compute_generative_perplexity(
        self, text_samples: typing.List[str], retokenize: bool = True, max_length: typing.Optional[int] = None
    ) -> None:
    ...
    def _ddpm_update(self, x, t, dt, labels, init_x=None):
        sigma_t, _ = self.noise(t)
    ...
    def _validate_init_x(self, init_x, *batch_dims):
        assert init_x.shape == batch_dims, (init_x.shape, batch_dims)
    ...
    @torch.no_grad()
    def _sample(self, num_steps=None, eps=1e-5, labels=None, init_x=None):
        ...
        if init_x is None:
            ...
            self._validate_init_x(init_x, batch_size_per_gpu, self.config.model.length)
    ...
    def restore_model_and_sample(self, num_steps, eps=1e-5, labels=None, init_x=None):
        """Generate samples from the model."""
        # Lightning auto-casting is not working in this method for some reason
        if self.ema:
            self.ema.store(itertools.chain(self.backbone.parameters(), self.noise.parameters()))
        ...
        self.backbone.eval()
        self.noise.eval()
        samples = self._sample(num_steps=num_steps, eps=eps, labels=labels, init_x=init_x)
        if self.ema:
            self.ema.restore(itertools.chain(self.backbone.parameters(), self.noise.parameters()))
        self.backbone.train()
        self.noise.train()
    ...
    def _forward_pass_diffusion(self, x0, labels):
        ...
        model_output = self.forward(xt, unet_conditioning, labels)
    ...
    def _loss(self, x0, attention_mask, labels):
        (input_tokens, output_tokens, attention_mask) = self._maybe_sub_sample(x0, attention_mask)

        if self.parameterization == "ar":
            logprobs = self.backbone(input_tokens, None, labels=labels)
            loss = -logprobs.gather(-1, output_tokens[:, :, None])[:, :, 0]
        else:
            loss = self._forward_pass_diffusion(input_tokens, labels)
    ...
    def sample_subs_guidance(self, n_samples, stride_length, num_strides, dt=0.001):
        ...
        sequence_lengths = (
            (np.concatenate(intermediate_tokens, axis=1)[:, 1:] == self.tokenizer.eos_token_id).cumsum(-1) == 0
        ).sum(-1)
